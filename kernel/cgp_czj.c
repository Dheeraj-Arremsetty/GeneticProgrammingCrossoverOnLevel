...
static double GLOBAL_LEVEL_VALUE = 0.3;
...
/////////////////
int MAX_TREE_SIZE = 15;
int LEVEL_COUNTER = 0;
int test = 0;
////////////////////

static int markXNodes_recurse_dheeraj( lnode **t, DEST_LEVEL )
/* assume Function_czj and Argument_czj are set to indicate dest point */
/* mark and count all feasible source nodes for crossover in tree */
/* marking is done with the corresponding weights w/r to dest parent */
/*   and wheel values are accumulated */
...
{
  ////////////////////DHEERAJ CHANGES///////////////////
    double calculated_probability_with_level = DEST_LEVEL - LEVEL_COUNTER;
    if(calculated_probability_with_level < 0){
      calculated_probability_with_level = calculated_probability_with_level * -1;
    }
    calculated_probability_with_level = calculated_probability_with_level / MAX_TREE_SIZE;
    calculated_probability_with_level = 1 - calculated_probability_with_level;
    // calculated_probability_with_level = 1;
    printf("******************************************\n" );
    printf("MAX_TREE_SIZE = %d\n",MAX_TREE_SIZE );
    printf("DEST_LEVEL = %d\n",DEST_LEVEL );
    printf("SUB_LEVEL = %d\n",LEVEL_COUNTER );
    printf("TEST = %d\n",test );
    printf("(**t).LEVEL_TREE = %d\n",(**t).LEVEL_TREE );
    printf("calculated_probability_with_level = %f\n",calculated_probability_with_level );
    printf("*******************************************\n" );
  ////////////////////////////////////////
  ...
  { case FUNC_DATA: case EVAL_DATA:
      ...
      { WghtsInt+=wght;//*calculated_probability_with_level;
        // printf("WghtsInt = %f\n",wght );
        total=1;
      }
      *wheelInt_czj=WghtsInt;
      *wheelExt_czj=WghtsExt;
      for (j=0; j<f->arity; ++j)
      {
        (**t).LEVEL_TREE = LEVEL_COUNTER;//(**t).LEVEL_TREE+1;
        LEVEL_COUNTER = LEVEL_COUNTER+1;
        test++;
        total+=markXNodes_recurse_dheeraj(t, DEST_LEVEL);    /* t has already been advanced */
      }
    ...
    case FUNC_EXPR: case EVAL_EXPR:
      ...
      *wheelExt_czj=WghtsExt;
      ...
      { ++*t;                   /* skip the pointer over the skipsize node. */
        ...
        LEVEL_COUNTER = LEVEL_COUNTER+1;
        test++;
        total+=markXNodes_recurse_dheeraj(t, DEST_LEVEL);
      }
      break;
  } /* switch */
  return total;
...
int markXNodes_dheeraj( lnode *data, int DEST_LEVEL )
/* assume Function_czj and Argument_czj are set to indicate dest point */
/* mark all nodes in tree which are feasible sources with their wghts */
/*   while contructing the wheels for internal and external nodes */
/* accummulate total wght marked in WghtsInt and WghtsExt */
...
{ lnode *t=data;
  ...
  LEVEL_COUNTER = 0;
  test = 0;
  return markXNodes_recurse_dheeraj (&t, DEST_LEVEL);
}
